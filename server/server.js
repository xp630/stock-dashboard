...
app.get('/api/search', async (req, res) => {
  try {
    ...
    const url = `${EF_BASE}/ulist.np/get?pn=1&pz=20&po=1&np=1&fltt=2&invt=2&fid=f3&fs=m:0+t:6,m:0+t:80,m:1+t:2,m:1+t:23&fields=f12,f13,f14,f3`;
    ...
    if (data.data && data.data.diff) {
      ...
      res.json({ data: { diff: filtered.slice(0, 10) } });
    } else {
      res.json({ data: { diff: [] } });
    }
  } catch (error) {
